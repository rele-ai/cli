...
class ComponentsClient extends BaseClient {
   ...
   */
  getVersionId(version, searchGlobal=false, getMultiple=false) {
    if (!version) {
...
const generateServiceClass = (type) => {
  ...
  const componentClass = class extends ComponentsClient {
    ...
    async getByKey(key, conds = [], version = "") {
      ...
      if (version) {
        conditions.push(toBytes(["version", "==", await this.getVersionId(version, true)]))
      }
