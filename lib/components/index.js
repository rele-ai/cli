...
const generateServiceClass = (type) => class extends ComponentsClient {
   ...
   * Returns a list of ${type}.
   *
   * @param {Array<string>} orgs - user's org ids.
   * @returns {Array.<object>} - List of configs.
   */
  list(orgs) {
   ...
   *
   * @param {string} key - Config key.
   * @param {Array.<string>} orgs - Org ids.
   * @param {string} keyName - Config key name.
   * @returns {object} - Config.
   */
  async getByKey(key, orgs, keyName) {
    // set conditions array
    // pulls all org that related to the user's
    // org id, or to global org.
    const conditions = [
    ...
    // check valid length
    if (items[this._type].length < 2) {
      if (items[this._type].length === 1) {
        return items[this._type][0]
      }
    } else {
