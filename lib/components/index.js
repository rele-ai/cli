...
class ComponentsClient extends BaseClient {
  ...
  /**
   * checkIfVersionExists take a version number and check
   * if its already Exists on DB
   *
   * @param {string} version
   * @returns {Boolean}
   */
  async checkIfVersionExists(version) {
    // check if version exsits
    const versionRecord = await this._list(
      this.proto.Versions.service.List,
      [
        toBytes(["version", "==", version])
      ],
    ...
    return {
      isExists: Boolean(versionRecord.length),
      versionId: versionRecord.length ? versionRecord[0].id : ""
    }
  }
...
const generateServiceClass = (type) => class extends ComponentsClient {
  ...
  async create(config, version) {
    ...
    // check if exists
    const { isExists, versionId } = await this.checkIfVersionExists(versionNumber)

    if (!isExists) {
      // create version
      const versionRes = await super.create(
        this.proto.Versions.Create,
        {
   ...
   * @returns {object} - Config.
   */
  async getByKey(key, conds = [], searchByVersion = true, version = "") {
    // set conditions array
    // pulls all org that related to the user's
    // org id, or to global org.
    let conditions = [
    ...
    if (searchByVersion) {
      conditions.push(toBytes(["version", "==", await this.getVersionId(version)]))
    }
  ...
  async updateByKey(key, config, cond = []) {
    // get object by key
    const object = await this.getByKey(key, cond)
