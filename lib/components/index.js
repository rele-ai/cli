...
class ComponentsClient extends BaseClient {
  ...
  async checkIfVersionExists(version) {
    ...
    const versionRecords = await this._list(
      this.proto.Versions.service.List,
      {
        conditions: [
          toBytes(["version", "==", version])
    ...
    return {
      isExists: Boolean((versionRecords || []).length),
      versionId: (versionRecords || []).length ? versionRecords[0].id : ""
    }
  }
...
const generateServiceClass = (type) => class extends ComponentsClient {
  ...
  async create(config, version) {
    ...
    // check if exists
    let { isExists, versionId } = await this.checkIfVersionExists(versionNumber)
