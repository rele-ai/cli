...
function versionSort (versions, opts) {
  ...
  lenStore.stageName = new Array(new Set(lenStore.stageName.flat()))
  ...
  len.number = Math.max(...lenStore.number).toString().length
  // Biggest number of sequence
  len.sequence = Math.max(...lenStore.sequence)
  // TODO: commenting
  len.stageName = lenStore.stageName.length.toString().length
  // TODO: commenting
  len.stageNumber = Math.max(...lenStore.stageNumber).toString().length
  ...
  v.forEach(function (_version, _i) {
    ...
    var stageNameIndex = lenStore.stageName.indexOf(stageName)
